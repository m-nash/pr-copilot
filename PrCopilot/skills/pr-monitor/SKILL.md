...
- **Reply composition** — after the agent finishes addressing a comment: if the agent does **not** provide `data.reply_text`, the **server composes the reply via sampling** with professional tone and commit linking; if the agent **does** provide `data.reply_text`, that text is used as the reply and the server posts it.
